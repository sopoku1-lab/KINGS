...
class Employee:
    ...
    def add_employee(self, first_name, last_name, email, department, hire_date):
        self.db.execute((first_name, last_name, email, department, hire_date), commit=True)

    def view_training_history(self, employee_id):
        self.db.execute((employee_id,))
...
class Course:
    ...
    def add_course(self, name, description, mandatory, start_date, end_date):
        self.db.execute((name, description, mandatory, start_date, end_date), commit=True)

    def update_course(self, course_id, new_name=None, new_description=None):
        if new_name:
            self.db.execute("UPDATE courses SET CourseName = ? WHERE CourseID = ?",
                            (new_name, course_id), commit=True)
        if new_description:
            self.db.execute("UPDATE courses SET Description = ? WHERE CourseID = ?",
                            (new_description, course_id), commit=True)
...
class Enrollment:
    ...
    def enroll_employee(self, employee_id, course_id):
        today = date.today()
        self.db.execute((employee_id, course_id, today), commit=True)

    def mark_completed(self, enrollment_id, score):
        today = date.today()
        self.db.execute((today, score, enrollment_id), commit=True)
...
class Report:
    ...
    def create_report(self, manager_id, name):
        today = date.today()
        self.db.execute( (manager_id, name, today), commit=True)

    def delete_report(self, report_id):
        self.db.execute("DELETE FROM reports WHERE ReportID = ?", (report_id,), commit=True)
...
def main():
    ...
    while True:
        ...
        if choice == "1":
            ...
            enrollment.enroll_employee(eid, cid)
